...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
     ...
     echo "create database schoolDB" | mysql -u root -p12345678
     mysql -u root -p12345678 schoolDB < /vagrant_scripts/db-simpleapp.sql
     SHELL


  config.vm.provision "shell",
    inline: "sh /vagrant_scripts/setup-tomcat.sh"
end
